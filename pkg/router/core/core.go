...
func (c *Context) JSONResult(code int, msg string, result ...interface{}) {
	j := new(JSONResult)
	j.Code = code
	j.Message = msg
	if len(result) > 0 {
...
func (c *Context) JSONResultRaw(code int, msg string, result []byte) {
	j := new(JSONResult)
	j.Code = code
	j.Message = msg
	j.Data = json.RawMessage(result)
	c.JSON(http.StatusOK, j)
	return
}
