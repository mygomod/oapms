...
import { Card, message, Button, Divider, Modal } from 'antd';
...
const handleCreate = async (values) => {
  ...
  try {
    const resp = await request("/api/admin/role/create" , {
      method: 'POST',
      data: {
        ...values,
      },
...
const handleUpdate = async (values) => {
  ...
  try {
    const resp = await request("/api/admin/role/update" , {
      method: 'POST',
      data: {
        ...values,
      },
...
export default class List extends React.Component {
  ...
  componentDidMount() {
    Promise.all([AppSelectArr()]).then((resultList) => {
      this.setState({
        done: true,
        selectData: resultList[0].data
      })
    })
  }

  render() {
    const { done,selectData,appId } = this.state
        ...
        },
        {
          ...
          render: (value, record) => (
            <Fragment>
              <a
                onClick={() => {
                  request('/api/admin/role/menus?id='+record.id).then((response) => {
